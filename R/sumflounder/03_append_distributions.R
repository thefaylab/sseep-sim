...
#read species data
species <- "summerflounder" # name of species to be simulated
season <- "spring" #survey season
...
preds <- readRDS(file = here(sdmtmb.dir, "sumflounder", "data", str_c(season, "_grid_preds_sf_1.rds", sep = ""))) |>
  rename(
    strat = STRATUM,
    depth = AVGDEPTH,
    x = X,
...
for (i in seq_along(nsims)) {
  ...
  pop_i  <- readRDS(file.path(pop.dat, sprintf("%s_pop_%03d.rds", species, i)))
  Nage_i <- readRDS(file.path(Nage.dat, sprintf("%s_Nage_%03d.rds", species, i)))
...
for (i in 1:100) {
  ...
  saveRDS(dist, here(dist.dat, sprintf("%s_%s_%03d_dist-only.rds", species, season, i)))
}


#Check
pop84sf <- readRDS(here("data", "rds", "dists", "summerflounder_spring_084_abund-dist.rds"))
...
pop84sf$sp_N |>
  ...
  facet_wrap(~age) +
  labs(title = "Pop 84: Spatial Distribution by Age", fill = "Abundance") +
  theme_minimal()
...
dist_84sf <- readRDS(here("data", "rds", "dists", "summerflounder_spring_084_dist-only.rds"))
ggplot(dist_84sf) +
