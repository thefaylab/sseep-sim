...
#TRUECVboxplot
ggplot(true_cv_plot, aes(x= factor(year), y = true_cv, fill = scenario)) +
  geom_boxplot(outlier.shape = NA, color = "black") +
  ylim(0, 0.2) +
  labs(title = " ", subtitle = "Summerflounder - Spring",
...
TRUECVboxplot_merged <- ggplot() +
  ...
  labs(title = "", subtitle = "Summerflounder - Spring", x = "Year", y = "CV", fill = "Scenario") +
  scale_fill_manual(values = c(
    "Status Quo"       = "#9B5F6B",
    "Preclusion"       = "#8C2B0E",
    "Reallocation"     = "#C5692D",
