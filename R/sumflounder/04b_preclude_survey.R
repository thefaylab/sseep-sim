...
#Compare
survdat_sf_precl_1 <- readRDS(here(survdat, "summerflounder_spring_001_25_precl_survey.rds"))

#Count
s.pre_sf <- survdat_sf_precl_1

# Count sets by sim, year
tow_counts_pre_sf <- s.pre_sf |>
  count(sim, year, name = "n_sets") #can add strat to check


print(tow_counts_pre_sf)


#Average number of tows per year across all simulations
avg_tows_per_year_pre_sf <- s.pre_sf |>
  count(sim, year) |>
  group_by(year) |>
  summarise(
    mean_tows = mean(n),
...
print(avg_tows_per_year_pre_sf)
