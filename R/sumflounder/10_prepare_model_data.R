...
## lat long in meters and tranformed
survdat_sq_ll <- lapply(survdat_sq, function(df) {
  df <- as.data.frame(df)
  df <- add_latlon(df)
  return(df)
})
...
for (p in unique(tb_sq$pop)) {
  for (s in unique(tb_sq$sim)) {
   ...
   # file_name <- sprintf("spring_sq_pop%03d_sim%02d.rds", p, s) #default is fall
...
for (s in unique(tb_precl$sim)) {
  ...
  #  file_name <- sprintf("spring_precl_pop%03d_sim%02d.rds", p, s) #default is fall
    file_path <- file.path(mods.data, file_name) # Save full path
