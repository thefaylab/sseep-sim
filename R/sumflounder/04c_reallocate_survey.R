...
for (i in this_chunk) {
  ...
  wind_tows <- sq_survey$setdet |> filter(AREA_CODE == 1, year >= 6)
  wind_cells <- wind_tows |> group_by(sim, year, strat) |> distinct(cell)
  wind_strat <- unique(wind_cells$strat)

  # Filter grid outside wind areas for affected strata
  grid_outside <- grid |> filter(AREA_CODE == 2, strat %in% wind_strat)
  out_wa_grid <- grid_outside |> group_by(strat) |> nest()
  ...
  join_data <- left_join(wind_summ, out_wa_grid, by = "strat") |>
 ...
 #Check
new_locs_sf <- readRDS(here(survdat, "summerflounder_spring_001_25_locs_survey.rds"))


# Tows per year
new_locs_sf |> count(year)


# Visualize them
library(ggplot2)
ggplot(new_locs_sf, aes(x, y)) +
...
survdat_reall_sf <- readRDS(here(survdat, "summerflounder_spring_001_25_reall_survey.rds"))
survdat_reall_sf


#Count
s_real_sf <- survdat_reall_sf


# Count sets by sim, year
tow_counts_sf <- s_real_sf |>
...
avg_tows_per_year_sf <- s_real_sf |>
  count(sim, year) |>
  group_by(year) |>
  summarise(
    mean_tows = mean(n),
