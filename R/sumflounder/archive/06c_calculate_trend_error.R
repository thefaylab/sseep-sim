...
ggplot(indices_lm) +
  ...
  labs(x = "Scenario", y = "Linear regression slope estimate",
       title = str_c("Distribution of linear regression slope estimates survey indices"),
  ...
  annotate("text", y = 0, x = 2, label = str_c("Relative true abundance slope: ", round(trueN_lm$estimate[1], 2)),
...
ggplot(slope_errors) +
  ...
  labs(x = "Scenario", y = "Linear regression slope",
    title = str_c("Errors of linear regression slope estimates"), subtitle = "Summerflounder - Spring") +
  theme_bw() +
  theme( text = element_text(size = 14),
         axis.title = element_text(size = 14),
         plot.title = element_text(size = 16, face = "bold"),
...
ggplot(slope_errors) +
  ...
  labs(title = "Relative errors of linear regression slope estimates", subtitle = "Summerflounder - Spring",
...
ggplot(slope_errors) +
  geom_boxplot(aes(x = scenario, y = abs_rel_err, fill = fct_inorder(scenario)),
               ...
               outlier.shape = NA,
               color = "black") +
  labs(title = "Absolute relative errors of linear regression slope estimates", subtitle = "Summerflounder - Spring",
       x = "Scenario", y = "Linear regression slope", fill = "Scenario") +
  scale_fill_manual(values = c("Status Quo" = "salmon",
                               "Preclusion" = "goldenrod",
                               "Reallocation" = "steelblue")) +
