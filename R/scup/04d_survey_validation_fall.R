...
grid_xy <- readRDS(here("data", "rds", "survey_grid_all_122024.rds")) |>
  rename(strat = STRATUM,
         x = X,
         y = Y,
         depth = mean_2) |>#, # rename to match column calls within SimSurvey functions
...
summary(grid_xy$depth)


# load spring model predictions
fall_preds <- readRDS(here(sseep.analysis, "sdmtmb", "scup", "data","fall_grid_preds_tw_2.rds")) #based on filtered mean depth.


fall_preds <- fall_preds |>
  dplyr::select(!c(median_1, mean_1, n1, Cell_Area, median_2,mean_2, n2, New_area, Perc_reduction))

source(file = here(sseep.analysis,"sdmtmb", "R", "plot_fns.R"))
plot_preds(fall_preds, exp(est)) + scale_fill_viridis_c(trans = "sqrt", option = "H") +
...
summary(preds_5yr$AVGDEPTH)

# predicted numbers at age based on simulated N0 and sdmtmb predictions
dist <- sdmTMB::replicate_df(preds_5yr, "age", c(0:7)) |> # replicate the predictions over each age, so there are distributions for each age
...
no_dist <- dplyr::anti_join(grid_xy, preds_5yr, by = "cell") |>
...
#pop <- readRDS(here("R", "scup", "data", "forced-pop-dist.rds"))


summary(pop$sp_N$depth)
...
ggplot() +
  ...
  coord_sf() +
  geom_point(data = data, aes(x*1000,y*1000, size = n, color = n)) +
  facet_wrap(~year) +
  scale_color_continuous(type = "viridis") +
  scale_size_continuous(guide = "none") +
  theme_bw() +
  labs(x = "Longitude", y = "Latitude", color = "Number", title="Sim Catch Rate", subtitle="mean filtered") +
...
survdat_sq <- survdat_sq[[1]]

fixed_grid <- survdat_sq$grid_xy

# Access the grid from the pop object
pop_grid <- pop$grid_xy
...
saveRDS(surv_dat_fixed, here("R", "scup", "data", "surv_dat_fixed.rds"))
surv_dat_fixed<- readRDS(here("R", "scup","data", "surv_dat_fixed.rds"))


# Extract sampled locations from surv_dat_fixed
sampled_locations <- surv_dat_fixed$grid$cell  # Modify based on the structure of surv_dat_fixed


#Data frame for fixed and sampled locations
fixed_df <- data.frame(x = fixed_grid$x, y = fixed_grid$y)
sampled_df <- data.frame(x = surv_dat_fixed$grid_xy$x, y = surv_dat_fixed$grid_xy$y)

# Plot the fixed and sampled locations
ggplot() +
  geom_point(data = fixed_df, aes(x = x, y = y), color = "blue", size = 3, alpha = 0.5) +
  geom_point(data = sampled_df, aes(x = x, y = y), color = "red", size = 2, alpha = 0.7) +
  labs(title = "Fixed vs Sampled Locations",
       x = "Long", y = "Lat")


surv_fixed_setdet <- surv_dat_fixed$setdet

summary(surv_fixed_setdet$depth)
...
prop_zero <- test_data |> #test data is for random survey loc
...
prop_zero2 <- test_data2 |> #test_data2 is for fixed survey loc
  group_by(TYPE, replicate, year) |>
  nest() |>
  mutate(zero = map(data, ~filter(., n == 0) |> nrow()),
         total = map(data, ~nrow(.))) |>
