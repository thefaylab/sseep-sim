...
ihat_sq_all <- map2_dfr(survdat_sq, seq_along(survdat_sq), function(surv, pop_num) {
  surv$setdet |> #it is already a data.table
    as_tibble() |>
...
indices2 <- bind_rows(ihat_sq_all,ihat_precl_all,ihat_hybrid_all)
...
indices_sub <- filter(indices2, pop %in% pops_to_plot)
indices_sub_sim <- filter(indices2, pop %in% pops_to_plot, sim %in% sim_to_plot)
