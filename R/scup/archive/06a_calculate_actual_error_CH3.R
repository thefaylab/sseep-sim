...
indices_2 <- readRDS(here(surv.prods, str_c(species, season, "all-ihat.rds", sep = "_")))
...
errors_plot <- errors %>%
  filter(
    (scenario == "Status Quo") |
      (scenario %in% c("Preclusion", "Reallocation", "Ratio estimator") & year >= 6)
  ) |>
  mutate(
    scenario = factor(
      scenario,
      levels = c("Status Quo", "Preclusion", "Reallocation", "Ratio estimator")
    )
  )
