...
## ESTIMATION ERROR ####
errors_plot <- errors %>%
  filter((scenario == "Status Quo") | (scenario %in% c("Preclusion", "Reallocation", "Ratio estimator", "Model based", "Model based wind") & year >= 6)) |>
  mutate(scenario = factor(scenario, levels = c("Status Quo", "Preclusion", "Reallocation", "Ratio estimator", "Model based", "Model based wind")))
