...
# turn the model fitting into a function
# turned off some of your error checking that didn't fit, you'd want to re-enable these
fit_estimator <- function(pop = 1, sim = 1, mods.data.dir = here(), fit.out.dir = here()) {
  ...
  # Skip if already fit
  #if (file.exists(out_path)) return(NA)
...
#create 'safe' version of the function call
safe_fit <- safely(fit_estimator, otherwise = NULL, quiet = TRUE)

#one function call
pop=1
sim=1
system.time(
safe_fit(pop = pop, sim = sim,
         mods.data.dir = mods.data.dir,
         fit.out.dir = fit.out.dir)
)

#set up futures multisession
ncores <- parallel::detectCores()
plan(multisession, workers = ncores-1)
#multiple function calls using futures to parallelize (here 5 simulations)
system.time(
  future_map2(as.list(rep(100,5)), as.list(rep(1,5)),
              ~safe_fit(pop = .x, sim = .y,
                       mods.data.dir = mods.data.dir,
                       fit.out.dir = fit.out.dir))
)
