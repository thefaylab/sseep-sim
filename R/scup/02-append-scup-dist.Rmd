...
preds_3yr <- fall_preds |>
  filter(EST_YEAR %in% c(2018:2021)) |>
...
```{r}
dist <- sdmTMB::replicate_df(preds_3yr, "age", c(1:6)) |> # replicate the predictions over each age, so there are distributions for each age
