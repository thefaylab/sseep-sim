...
pop <- readRDS(here("R", "scup", "data", "scup_dist_fall.rds"))
surv_dat1 <- readRDS(here("R", "scup", "data", "scup_survdat_25sims_fall.rds"))
...
out_wa_grid does not contain NULL strata 
```{r}
wind_summ <-  wind_tows |>
   #filter(strat != 3200) |> 
   group_by(sim, year, strat ) |>
...
```{r, include = FALSE}
saveRDS(new_locations, here("R", "scup", "data", "new_locations_fall.rds"))
saveRDS(surv_reallocation, here("R", "scup", "data", "scup_survreallocation_25sims_fall.rds"))
```
